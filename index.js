...
class S3Downloader {
	constructor(options) {
		...
		this.installDependencies = options.installDependencies === false ? false : true;
		this.s3 = new AWS.S3({
	...
	downloadAppZip() {
		return new Promise((res, rej) => {
			...
			request.createReadStream().pipe(file).on('close', res).on('error', rej);
		});
	}
	...
	unzipApp() {
		...
		return this.exec('unzip -q ' + zipPath).then(() => {
